import { GraphData, GraphDebt, GraphUser } from "@shared/schema";
...
export class LoanGraph {
  ...
  // Get user ID by name
  getUserId(user: string): number | undefined {
    return this.userToID.get(user);
  }
  ...
  removeUser(user: string): void {
    if (!this.userToID.has(user)) return;
    
    const userId = this.userToID.get(user);
    
    // Remove edges involving this user
    this.adj.delete(userId!);
    for (const [u, adjMap] of this.adj.entries()) {
      adjMap.delete(userId!);
    }
    
    // Remove from debtMap
    for (const key of this.debtMap.keys()) {
      if (key.includes(user)) {
        this.debtMap.delete(key);
      }
    }
    
    // Remove from userToID and idToUser
    this.userToID.delete(user);
    
    // Rebuild idToUser and fix IDs
    this.idToUser = [];
    this.userToID = new Map();
    this.userCount = 0;
    
    // Re-add remaining users
    for (const [name, _] of this.userToID.entries()) {
      this.addUser(name);
    }
  }
